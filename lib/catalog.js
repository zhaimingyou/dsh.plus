...
function deriveInstall(repositoryUrl) {
...
async function loadStars() {
    const map = new Map();
    try {
        const rows = (await fetchJson(`${CATALOG_ORIGIN}/plugin-index.json`));
        ...
        for (const row of rows) {
            ...
            if (slug && typeof stars === 'number' && Number.isFinite(stars))
                map.set(slug, stars);
        }
    }
    catch (cause) {
...
async function loadProviderItems(locale) {
    const items = [];
    let cursor;
    for (let page = 0; page < MAX_PAGES; page++) {
        ...
        const payload = (await fetchJson(`${CATALOG_ORIGIN}/v1/plugins?${params.toString()}`));
...
function toEntry(raw, stars) {
    if (raw === null || typeof raw !== 'object')
        return null;
    const item = raw;
    const repository = safeUrl(item.repository?.url);
    const id = safeString(item.id, 120);
    const name = safeString(item.displayName ?? item.name, 120);
    const summary = safeString(item.summary, 300);
    if (!repository || !id || !name || !summary)
        return null;
    const categories = Array.isArray(item.categories)
        ? item.categories
            .filter((c) => typeof c === 'string' && CATEGORY_ID_RE.test(c))
            .slice(0, 8)
        : [];
    const description = safeString(item.description, 4000);
    const cover = safeUrl(item.media?.icon?.url);
    ...
    return {
        ...
        publisher: safeString(item.publisher?.name, 120) || undefined,
